function Start-ServerInstall {
    ...
    $gamePath_literal   = $gamePath
    $targetName_literal = $Target.Name
    $targetPath_literal = $Target.Path
    $targetId_literal   = $Target.ServerId
...
`$host.ui.RawUI.WindowTitle = 'Installing: $targetName_literal'
Write-Host 'Installing: $targetName_literal' -ForegroundColor Cyan
Write-Host ''

try {
    `$exe = Install-SteamCMD -Path '$steamDir'
    Install-GameServer -SteamCmdPath `$exe -AppId '$targetAppId' -InstallDir '$gamePath_literal' -SteamCmdDir '$steamDir' -ScriptId '$targetId_literal'
    Update-ServerStatus -ServerId '$targetId_literal' -Status 'Installed'
    Write-Host ''
    Write-Host 'Installazione completata con successo.' -ForegroundColor Green
    Write-Log 'Installazione completata: $targetPath_literal' 'INFO'
}
catch {
    Update-ServerStatus -ServerId '$targetId_literal' -Status 'Error'
